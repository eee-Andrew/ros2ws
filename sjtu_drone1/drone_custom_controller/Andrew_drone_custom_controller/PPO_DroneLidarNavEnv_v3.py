...
class DroneLidarNavEnv(Node, gym.Env):
    def __init__(self):
        ...
        # Takeoff flag to ensure takeoff happens only once per episode
    ...
    def reset(self, seed=None, options=None):
        ...
        # Perform takeoff after environment is ready
        self.takeoff()
    ...
    def takeoff(self):
        ...
        # Wait for takeoff to complete and monitor altitude
        takeoff_timeout = 10.0  # seconds
        start_time = time.time()
        target_altitude = 0.8  # Minimum altitude to consider takeoff successful
        
        while time.time() - start_time < takeoff_timeout:
            rclpy.spin_once(self, timeout_sec=0.1)
            
            if self.current_position[2] >= target_altitude:
                self.get_logger().info(f"Takeoff completed! Current altitude: {self.current_position[2]:.2f}m")
        ...
        if not self.takeoff_completed:
            self.get_logger().warn(f"Takeoff may not have completed. Current altitude: {self.current_position[2]:.2f}m")
